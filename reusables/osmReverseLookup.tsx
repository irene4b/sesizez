...
export const osmReverseLookup = async (props: Props): Promise<reportedLocation> => {
  ...
  else if(response.data.address.shop) adresaLinie1 += ` (pe langa ${response.data.address.shop})`;
  else if(response.data.address.amenity) adresaLinie1 += ` (pe langa ${response.data.address.amenity})`;
  ...
  else if(response.data.address.district) county = response.data.address.district;
